...
#[cfg(test)]
pub(in crate::native) fn is_phi_line(line: &str) -> bool {
    let t = line.trim();
    // `%dst = phi <ty> [ ... ], [ ... ]`
    if let Some((_, rhs)) = t.split_once('=') {
        return rhs.trim_start().starts_with("phi ");
    }
    false
}

/// Replace `label <from>` with `label <to>` in a terminator line, matching `from` only at an
/// identifier boundary so e.g. `%bb1` does not match inside `%bb10`. `from`/`to` include the `%`.
#[cfg(test)]
pub(in crate::native) fn redirect_label(line: &str, from: &str, to: &str) -> String {
...
#[cfg(test)]
pub(in crate::native) fn is_ident_char(c: char) -> bool {
